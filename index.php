...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  if (!empty($_COOKIE[session_name()]) && session_start() && !empty($_SESSION['login'])) {
    ...
    try {
      ...
      // и достает из нее все значения, где userId = логину пользователя.
      $stmt = $db->prepare("SELECT * FROM table5 WHERE userId = ?");
...
// в базу данных.
  ...
  if (!empty($_COOKIE[session_name()]) && session_start() && !empty($_SESSION['login'])) {
    ...
    try {
      // Подготавливаем SQL запрос, где вопросы заменятся на значения.
      // Запрос направляется в таблицу form5 с данными пользователей
      // и обновляет значения, кроме userId, где userId = логину пользователя.
      $stmt = $db->prepare("UPDATE table5 SET name = ?, email = ?, year = ?, sex = ?, limbs = ?, powers = ?, bio = ? WHERE userId = ?");
      // Заменяем вопросы на значения.
      $stmt->execute(array(
        $_POST['name'],
        $_POST['email'],
    ...
    try {
      // Подготавливаем SQL запрос, где вопросы заменятся на значения.
      // Запрос направляется в таблицу form5 с данными пользователей.
      $stmt_form = $db->prepare("INSERT INTO table5 SET userId = ?, name = ?, email = ?, year = ?, sex = ?, limbs = ?, powers = ?, bio = ?");
      // Заменяем вопросы на значения.
      ...
      $stmt_user = $db->prepare("INSERT INTO users SET login = ?, password = ?");
